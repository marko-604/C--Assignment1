...
int main() {
  ...
  while (true) {
    ...
    if (row == 'Z' && col == -1)
      break;
    gameMap.setCellToPath(row - 'A', col); // Should trigger notify()
    //gameMap.displayMap();
  }
  cout << "Set entry point (Row(A-Z) Column(0-9)): ";
  cin >> row >> col;
  gameMap.setEntryPoint(row - 'A', col); // Observer updates map view
  //gameMap.displayMap();

  cout << "Set exit point (Row(A-Z) Column(0-9)): ";
  cin >> row >> col;
  gameMap.setExitPoint(row - 'A', col); // Observer updates map view
  //gameMap.displayMap();
